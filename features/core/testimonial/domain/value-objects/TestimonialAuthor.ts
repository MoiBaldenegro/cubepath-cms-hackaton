...
export class TestimonialAuthor {
    ...
    private ensureIsValid(value: string): void {
        ...
        if (trimmedValue.length < 2) {
            throw new Error("TestimonialAuthor name is too short. It must be at least 2 characters.");
        }
        if (trimmedValue.length > 100) {
            throw new Error("TestimonialAuthor name is too long. It must be under 100 characters.");
        }
