...
export class UpdateTestimonial {
  ...
  async run(request: UpdateTestimonialRequest): Promise<void> {
    ...
    if (request.status) {
      if (
        !Object.values(TestimonialStatusEnum).includes(
          request.status as TestimonialStatusEnum,
        )
      ) {
        throw new Error(`Invalid status: ${request.status}`);
      }
      data.status = new TestimonialStatus(
        request.status as TestimonialStatusEnum,
      );
    }
    if (request.tags) {
    ...
    if (request.rating) {
      ...
      data.rating = new TestimonialRating(
        request.rating as TestimonialRatingEnum,
      );
    }
    if (request.category) {
      ...
      data.category = new TestimonialCategory(
        request.category as TestimonialCategoryEnum,
      );
    }
    if (request.imageUrl) {
